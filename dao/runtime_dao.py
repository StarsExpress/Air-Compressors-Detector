...
def read_runtime(logger=None):
    ...
    results_unprocessed = session.query(RuntimeDao.id, RuntimeDao.datetime, RuntimeDao.asset_code,
                                        RuntimeDao.runtime, RuntimeDao.status).filter(RuntimeDao.status == 0).all()
    session.close()
    ...
    if results_unprocessed is not None:
        for result in results_unprocessed:
            unprocessed_runtime_values_list.append([result.id, result.datetime, result.asset_code,
                                                    result.runtime, result.status])

    unprocessed_runtime_df = pd.DataFrame(unprocessed_runtime_values_list, columns=RUNTIME_COLUMNS_LIST)
    if len(unprocessed_runtime_df) <= 0:  # If unprocessed runtime data is empty, return empty data frame.
        if logger is not None:
    ...
    datetime_threshold = str(min(unprocessed_runtime_df['datetime']) - datetime.timedelta(days=ADDITIONAL_RUNTIME_DAYS))
    ...
    results_past = session.query(RuntimeDao.id, RuntimeDao.datetime, RuntimeDao.asset_code,
                                 RuntimeDao.runtime, RuntimeDao.status).filter(
                                (RuntimeDao.status == 1) & (RuntimeDao.asset_code.in_(asset_codes_list)) &
                                (RuntimeDao.datetime >= datetime_threshold)).all()
    session.close()
    ...
    if results_past is not None:
        for result in results_past:
            past_runtime_values_list.append([result.id, result.datetime,
                                             result.asset_code, result.runtime, result.status])

    past_runtime_df = pd.DataFrame(past_runtime_values_list, columns=RUNTIME_COLUMNS_LIST)
    ...
    runtime_df.sort_values(by=['datetime', 'asset_code'], ascending=[True, True], inplace=True)
    return runtime_df
...
def update_execution_status(runtime_df, load_date_str, logger=None):
    ...
    session = DBSession()
    for i, row in runtime_df.iterrows():  # Update last-7-day average, load date and status by filtering ID.
        session.query(RuntimeDao).filter(RuntimeDao.id == row['id']).update(
            {'last_7_day_avg': row['last_7_day_avg'], 'load_date': load_date_str, 'status': 1})
        session.commit()
    session.close()
