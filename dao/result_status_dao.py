...
def insert_and_update(result_status_df, load_date_str, logger=None):
    ...
    result_status_df['datetime'] = result_status_df['datetime'].astype(str)  # Turn timestamps to string.
    result_status_df['push_status'] = 0  # Default value of push status column.
    result_status_df['load_date'] = load_date_str
    result_status_df_col_names = ', '.join(list(result_status_df.columns))  # Column names in result status data.

    # String that represents SQL update syntax.
    update_str = ', '.join([''.join([l, ' = VALUES(', l, ')']) for l in list(result_status_df.columns)])

    if logger is not None:
        logger.write('debug', 'MySQL connection ready.')
    connection = engine.connect()

    for i, row in result_status_df.iterrows():  # For each row, put all values into a tuple.
        values_tuple = (row['id'], row['datetime'], row['asset_code'], row['tag_name'],
                        row['result_status'], int(row['push_status']), row['load_date'],)

        # Use insert on duplicate key to prevent duplicates from entering database.
        query = '''INSERT INTO ts_air_compressor_result_status (''' + result_status_df_col_names + ''')
...
def read_unsent_result_status(logger=None):
    ...
    unsent_results = session.query(ResultStatusDao.datetime, ResultStatusDao.tag_name, ResultStatusDao.asset_code,
                                   ResultStatusDao.result_status).filter(ResultStatusDao.push_status == 0).all()
    session.close()
    ...
    if unsent_results is not None:
        for unsent_result in unsent_results:
            unsent_result_status_values_list.append([unsent_result.datetime, unsent_result.tag_name,
                                                     unsent_result.asset_code, unsent_result.result_status])

    unsent_result_status_df = pd.DataFrame(unsent_result_status_values_list, columns=UNSENT_RESULT_STATUS_COLUMNS_LIST)
    if len(unsent_result_status_df) <= 0:  # If unsent result status data is empty, return empty data frame.
        if logger is not None:
    ...
    unsent_result_status_df.sort_values(by=['Time', 'Tag Name'], ascending=[True, True], inplace=True)
...
def update_push_status(sent_result_status_id_list, logger=None):
    ...
    session = DBSession()
    session.query(ResultStatusDao).filter(ResultStatusDao.id.in_(sent_result_status_id_list)).update({'push_status': 1})
    session.commit()
    session.close()
