...
def find_unprocessed_csv(lock):
    last_processed_txt_path = os.path.join(
        DATA_ACQUISITION_BASE_PATH, "last_processed_timestamp.txt"
    )
    ...
    last_processed_timestamp = datetime.datetime.strptime(
        last_processed_timestamp, "%Y-%m-%d %H:%M:%S"
    )
    last_processed_timestamp = last_processed_timestamp.replace(
    ...
    return (
        sorted(map(lambda x: os.path.join(SOURCE_FOLDER_BASE_PATH, x), match_list)),
