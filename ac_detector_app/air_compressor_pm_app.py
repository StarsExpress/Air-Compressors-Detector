...
class Application:
    ...
    def execute(self, lock):
        processing = AirCompressorMetaProcessor(self.logger)  # Air compressor meta processor.
        ...
        air_compressor_meta_data = air_compressor_meta_dao.read_air_compressor_meta(self.logger)
        if len(air_compressor_meta_data) <= 0:  # If meta data is empty.
            self.logger.write('error', 'App error: air compressor meta data empty.')
            return

        self.logger.write('debug', 'Step 1: calculate last 7-day average runtime for all records.')

        calculation_step = CalculationStep()
        calculation_step.conduct(runtime_data)
        runtime_data = calculation_step.runtime_df
        if len(runtime_data) <= 0:  # If runtime data is empty after last 7-day average calculation.
            print('App return: runtime data empty after last 7-day average calculation.')
            self.logger.write('warning', 'App return: runtime data empty after last-7-day average calculation.')
            return

        self.logger.write('debug', 'Step 2: apply DBSCAN clustering to pick out extreme positive points.')

        clustering_step = ClusteringStep()
        clustering_step.conduct(air_compressor_meta_data, runtime_data)  # Do DBSCAN clustering.
        ...
        load_date_str = datetime.datetime.now(tz=ZoneInfo(TIMEZONE)).replace(
            tzinfo=None).isoformat(sep=' ', timespec='seconds')
        runtime_dao.update_execution_status(runtime_data, load_date_str)  # Update runtime data status.
        ...
        self.logger.write('debug', 'Step 3: insert and update abnormal clusters from step 2.')
