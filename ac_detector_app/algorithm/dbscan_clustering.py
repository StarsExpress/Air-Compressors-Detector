...
class DBSCANClusterer:
    ...
    def __init__(self, runtime_data, logger=None):
        ...
        if self.logger is not None:
            self.logger.write('debug', 'Air compressor meta data stored into clustering class.')
        ...
        self.abnormal_cluster_df = pd.DataFrame()  # Data frame for abnormal cluster data.
    ...
    def fit_and_predict(self, meta_data):
        asset_codes_list = sorted(set(self.runtime_df['asset_code']))  # List of asset codes in runtime data.
        if len(asset_codes_list) <= 0:  # If asset codes list is empty.
            if self.logger is not None:
                self.logger.write('error', 'DBSCAN clustering stopped: air compressor asset codes list empty.')
        ...
        runtime_df = runtime_df[runtime_df['status'] == 0]  # Only keep runtime data with status = 0 after DBSCAN.
        ...
        if self.logger is not None:
            ...
            self.logger.write('debug', 'Clustering results stored into clustering class.')
            log_message = self.abnormal_cluster_df.to_string(justify=DF_ALIGNMENT, index=False)
