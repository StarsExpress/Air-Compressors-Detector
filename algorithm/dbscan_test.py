...
class DBSCANTester:
    ...
    def __init__(self, runtime_data):
        ...
        self.runtime_df = runtime_data.drop(
            columns=["id", "status"]
        )  # Store runtime data.

        self.abnormal_cluster_df = (
            pd.DataFrame()
        )  # Data frame for abnormal cluster details.
    ...
    def evaluate(
        self, meta_data, asset_codes_list=None, clustering_summary=True, export_csv=True
    ):
        ...
        # Iterate through air compressors' asset codes.
        ...
        if (
            clustering_summary
        ):  # When printing a data frame, let index starts from 1 if not empty.
        ...
        if export_csv:  # Save abnormal cluster data as csv.
            self.abnormal_cluster_df.to_csv(
                os.path.join(APP_BASE_PATH, "test", "air_compressor_alarms.csv"),
                index=False,
            )
...
if __name__ == "__main__":
    ...
    if len(runtime_data_main) > 0:  # If runtime data exist.
        meta_data_main = (
            air_compressor_meta_dao.read_air_compressor_meta()
        )  # Air compressor meta data.

        # Only use data since 2020.
        datetime_threshold = datetime.datetime(2020, 1, 1, 0, 0, 0)
        runtime_data_main = runtime_data_main[
            runtime_data_main["datetime"] >= datetime_threshold
        ]
